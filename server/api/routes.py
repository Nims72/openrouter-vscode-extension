...
_ACTIVE_MODEL_FILE = Path(__file__).parent.parent / ".active_model.json"


def _write_active_model(resolved: str, original: str) -> None:
    """Write the currently active model to a file for the VS Code extension to read."""
    name_lower = original.lower() if original else ""
    tier = "opus" if "opus" in name_lower else "haiku" if "haiku" in name_lower else "sonnet"
    try:
        _ACTIVE_MODEL_FILE.write_text(
            json.dumps({"model": resolved, "tier": tier, "original": original}),
            encoding="utf-8",
        )
    except OSError:
        pass
...
async def create_message(
    ...
    try:
        ...
        _write_active_model(
            request_data.resolved_provider_model or settings.model,
            request_data.original_model or "",
        )
